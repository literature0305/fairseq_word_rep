...
class MMapIndexedDataset(torch.utils.data.Dataset):
    ...
    def _do_init(self, path):
        ...
        # get word tokens for iwslt14 dataset 1.0
        # raise ValueError('self._path', self._path) # 'data-bin/wmt17_de_en/valid.de-en.de'
        # raise ValueError('self._path', self._path) #ValueError: ('self._path', 'data-bin/iwslt14.tokenized.de-en/valid.de-en.de')

        name_target_dirs = ['iwslt14.tokenized.de-en', 'wmt17_de_en']
        for name_target_dir in name_target_dirs:
            if name_target_dir in self._path:
                name_target_dir_new = name_target_dir + '_word'
                self._path_word = path.replace(name_target_dir,name_target_dir_new)
                self._index_word = self.Index(index_file_path(self._path_word))
                _warmup_mmap_file(data_file_path(self._path_word))
                self._bin_buffer_mmap_word = np.memmap(
                    data_file_path(self._path_word), mode="r", order="C"
                )
                self._bin_buffer_word = memoryview(self._bin_buffer_mmap_word)
                break
            else:
                self._index_word = None
                self._bin_buffer_word = None
    ...
    def __getitem__(self, i):
        ...
        # get word tokens for iwslt14 dataset 1.1
        if self._index_word is not None:
            ptr_word, size_word = self._index_word[i]
            np_array_word = np.frombuffer(
                self._bin_buffer_word, dtype=self._index_word.dtype, count=size_word, offset=ptr_word
            )
            if self._index_word.dtype != np.int64:
                np_array_word = np_array_word.astype(np.int64)
            
            # print('np_array', np_array, 'np_array_word', np_array_word)

            return [torch.from_numpy(np_array), torch.from_numpy(np_array_word)]
            # return torch.from_numpy(np_array)

        raise ValueError('if you want to train the model with auxiliary word level loss, not here maybe..')
        # print('np_array', np_array, 'self._bin_buffer',self._bin_buffer, 'count', size, 'offset', ptr)
